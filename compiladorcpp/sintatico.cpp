...
Parser::Parser(Scanner &scan)
    : scanner(scan), currentToken(TokenType::T_EOF, "", 0) {
  advance(); // avança para o primeiro token
}

// Avança para o próximo token
void Parser::advance() { currentToken = scanner.nextToken(); }
...
void Parser::error(const std::string &message) {
  throw std::runtime_error("Erro Sintatico na linha " +
                           std::to_string(currentToken.line) + ": " + message);
}
...
std::unique_ptr<Statement> Parser::parseStatement() {
  if (currentToken.type == TokenType::T_LET)
    return parseDeclaration();
  if (currentToken.type == TokenType::T_PRINTLN)
    return parsePrintStmt();
  if (currentToken.type == TokenType::T_IF)
    return parseIfStmt();
  if (currentToken.type == TokenType::T_WHILE)
    return parseWhileStmt();
  if (currentToken.type == TokenType::T_FN)
    return parseFnDecl();
...
// Analisa soma e subtração, mas primeiro tenta ler multiplicaões para ordem de
// operações
...
// Analisa fator (função responsável por analisar números, strings,
// identificadores e parênteses) Se achar parenteses resolve primeiro
