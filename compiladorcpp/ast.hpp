...
public:
  ...
  virtual std::string
  toJson() const = 0; // Exigência de melhoria: Exportação em JSON
  virtual void
  print(int indent = 0) const = 0; // Exibe a estrutura da árvore sintática de
                                   // forma identada no terminal
};
...
public:
  NumberExpr(std::string val)
      : value(val) {} // recebe inteiro e salva na memoria
...
public:
  ...
  void print(int indent = 0) const override {
    std::cout << std::string(indent, ' ') << "FloatExpr: " << value
              << std::endl;
  }
};
...
public:
  StringExpr(std::string val) : value(val) {} // recebe string e salva na
                                              // memoria
  std::string toJson() const override {
    return "{\"type\": \"String\", \"value\": \"" + escapeJson(value) + "\"}";
  }
  void print(int indent = 0) const override {
    std::cout << std::string(indent, ' ') << "StringExpr: \""
              << escapeJson(value) << "\"" << std::endl;
  }
};
...
public:
  ...
  } // recebe operaçao binaria e salva na memoria
  std::string toJson() const override {
    return "{\"type\": \"BinaryExpr\", \"op\": \"" + op +
           "\", \"left\": " + left->toJson() +
           ", \"right\": " + right->toJson() + "}";
  }
  void print(int indent = 0) const override {
...
public:
  ...
  std::string toJson() const override {
    ...
    return "{\"type\": \"LetDecl\", \"id\": \"" + id +
           "\", \"isMut\": " + mutStr + ", \"init\": " + initJson + "}";
  }
  void print(int indent = 0) const override {
    std::cout << std::string(indent, ' ') << "LetDeclStmt: " << id
              << (isMut ? " (mut)" : "") << std::endl;
    if (initializer) {
...
class AssignmentStmt : public Statement { // representa a atribuição de um valor
                                          // a uma variável que já foi declarada
  std::string id;             // nome da variavel
  std::unique_ptr<Expr> expr; // expressão que será atribuida
public:
  AssignmentStmt(std::string name, std::unique_ptr<Expr> e)
      : id(name), expr(std::move(e)) {}
  std::string toJson() const override {
    return "{\"type\": \"Assignment\", \"id\": \"" + id +
           "\", \"expr\": " + expr->toJson() + "}";
  }
  void print(int indent = 0) const override {
    std::cout << std::string(indent, ' ') << "AssignmentStmt: " << id
              << std::endl;
    expr->print(indent + 2);
  }
};
...
class PrintlnStmt
    : public Statement { // representa a impressão de um valor na tela
...
public:
  ...
  void print(int indent = 0) const override {
    std::cout << std::string(indent, ' ') << "PrintlnStmt" << std::endl;
...
class BlockStmt : public Statement { // representa um bloco de código
  std::vector<std::unique_ptr<Statement>>
      statements; // vetor de instruções tudo que está dentro de {}
public:
  ...
  void print(int indent = 0) const override {
    std::cout << std::string(indent, ' ') << "BlockStmt" << std::endl;
...
class IfStmt : public Statement {        // representa um if
  ...
  std::unique_ptr<BlockStmt> thenBranch; // bloco de código que será executado
                                         // se a condição for verdadeira
  std::unique_ptr<BlockStmt>
      elseBranch; // bloco de código que será executado se a condição for falsa
public:
  ...
  void print(int indent = 0) const override {
    ...
    std::cout << std::string(indent + 2, ' ') << "ThenBranch:" << std::endl;
    thenBranch->print(indent + 4);
    if (elseBranch) {
      std::cout << std::string(indent + 2, ' ') << "ElseBranch:" << std::endl;
...
public:
  ...
  std::string toJson() const override {
    return "{\"type\": \"FnDecl\", \"name\": \"" + name +
           "\", \"body\": " + body->toJson() + "}";
  }
  void print(int indent = 0) const override {
    std::cout << std::string(indent, ' ') << "FnDeclStmt: " << name
              << std::endl;
    body->print(indent + 2);
  }
};
...
class Program : public ASTNode { // representa o programa inteiro
  std::vector<std::unique_ptr<Statement>>
      statements; // vetor de instruções do programa
public:
  void addStatement(std::unique_ptr<Statement> stmt) {
    statements.push_back(std::move(stmt));
  }
